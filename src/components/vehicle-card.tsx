  ...
  return (
    <Card className="overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out flex flex-col h-full rounded-lg border group hover:border-primary">
      <CardHeader className="p-0">
        <Link href={`/inventory/${vehicle.id}`} className="block relative h-52 sm:h-60 w-full overflow-hidden">
          <Image
      ...
      <CardContent className="p-4 sm:p-6 flex-grow">
        ...
        <div className="space-y-1.5 text-xs sm:text-sm text-muted-foreground">
          <div className="flex items-center gap-2">
            <CalendarDays className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground/80 flex-shrink-0" />
            <span>{vehicle.year}</span>
          </div>
          <div className="flex items-center gap-2">
            <Gauge className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground/80 flex-shrink-0" />
            <span>{vehicle.mileage.toLocaleString()} miles</span>
          </div>
          <div className="flex items-center gap-2">
            <Gem className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground/80 flex-shrink-0" />
            <span>{vehicle.fuelType}</span>
          </div>
           <div className="flex items-center gap-2">
            <Settings2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground/80 flex-shrink-0" />
            <span>{vehicle.transmission}</span>
          </div>
          <div className="flex items-center gap-2">
            <Palette className="h-3.5 w-3.5 sm:h-4 sm:w-4 text-muted-foreground/80 flex-shrink-0" />
            <span>{vehicle.color}</span>
          </div>
        </div>
      </CardContent>
      <CardFooter className="p-4 sm:p-6 border-t bg-muted/10 dark:bg-muted/20">
        <Button asChild className="w-full font-medium text-sm sm:text-base">
